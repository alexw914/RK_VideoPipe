...
namespace vp_nodes {
    ...
    class vp_rk_second_yolo: public vp_secondary_infer_node
    {
    private:
        /* data */
    ...
    public:
        vp_rk_second_yolo(std::string node_name, std::string config_path, std::vector<int> p_class_ids_applied_to = std::vector<int>(), int min_width_applied_to = 0, int min_height_applied_to = 0);
        ~vp_rk_second_yolo();
    };
}